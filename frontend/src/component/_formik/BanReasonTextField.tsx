...
export const BanReasonTextField = <T,>({ paired = true }: { paired?: boolean }) => {
    const { values, touched, errors, handleChange } = useFormikContext<T & BanReasonTextFieldProps & BanReasonFieldProps>();

    const isError = useMemo(() => {
        if (paired) {
            return values.reason == BanReason.Custom && touched.reason_text && Boolean(errors.reason_text);
        }
    ...
    return (
        <TextField
            ...
            error={isError}
            helperText={touched.reason_text && errors.reason_text && `${errors.reason_text}`}
            variant="outlined"
        />
    );
};
