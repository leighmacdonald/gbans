...
export const ServerIDField = <T,>({ servers }: { servers: ServerSimple[] }) => {
    ...
    return (
        <Select<number> fullWidth value={values.server_id} name={'server_id'} id={'server_id'} onChange={handleChange} label={'Server'}>
            {servers.map((server) => {
                return (
...
export const ServerIDCell = ({ server_id, server_name }: { server_id: number; server_name: string }) => {
    const { setFieldValue, submitForm } = useFormikContext<ServerIDFieldProps>();
