...
export const SelectOwnField = <T,>({ disabled }: { disabled?: boolean }) => {
    const { values, touched, errors, handleChange } = useFormikContext<T & SelectOwnFieldProps>();
    return (
        <VCenterBox>
            <FormControl fullWidth disabled={disabled}>
                ...
                <FormHelperText>{touched.select_own && errors.select_own && `${errors.select_own}`}</FormHelperText>
            </FormControl>
        </VCenterBox>
    );
};
