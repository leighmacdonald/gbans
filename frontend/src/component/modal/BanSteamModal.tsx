...
import { apiCreateBanSteam, apiUpdateBanSteam, BanReason, BanType, Duration, SteamBanRecord } from '../../api';
...
export const BanSteamModal = NiceModal.create(({ steamId, reportId, existing }: BanModalProps) => {
        ...
        async (values: BanSteamFormValues) => {
            try {
                if (isUpdate && existing) {
                    const ban_record = await apiUpdateBanSteam(existing.ban_id, {
                        note: values.note,
    ...
    return (
        <Formik
            ...
            initialValues={{
                ...
                duration: existing?.ban_id && existing?.ban_id > 0 ? Duration.durCustom : Duration.dur2w,
                duration_custom: existing?.ban_id && existing?.ban_id > 0 ? existing?.valid_until : new Date(),
                note: existing?.note ?? '',
