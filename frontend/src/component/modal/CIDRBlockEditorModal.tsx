...
import {
    apiCreateCIDRBlockSource,
    apiUpdateCIDRBlockSource,
    CIDRBlockSource
} from '../../api';
import { Heading } from '../Heading';
import { EnabledField, EnabledFieldValidator } from '../formik/EnabledField';
import { NameField, NameFieldValidator } from '../formik/NameField';
import { URLField, URLFieldValidator } from '../formik/URLField';
...
export const CIDRBlockEditorModal = NiceModal.create(
    ({ source }: CIDRBlockEditorProps) => {
            ...
            async (values: CIDRBlockEditorValues) => {
                try {
                    if (values.cidr_block_source_id > 0) {
                        ...
                        const resp = await apiCreateCIDRBlockSource(
                            values.name,
                            values.url,
                            values.enabled
                        );
