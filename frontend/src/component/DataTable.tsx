...
interface EnhancedTableProps<TRecord> {
    ...
    numSelected: number;
    onRequestSort: (event: React.MouseEvent<unknown>, property: keyof TRecord) => void;
...
export function createDataTable<TRecord>() {
    ...
    function EnhancedTableHead<TRecord>(props: EnhancedTableProps<TRecord>) {
        ...
        const createSortHandler = (property: keyof TRecord) => (event: React.MouseEvent<unknown>) => {
    ...
    return ({connector, id_field, headers, heading}: TableProps<TRecord>) => {
        ...
        const handleRequestSort = (_: React.MouseEvent<unknown>, property: keyof TRecord) => {
                        ...
                        >
                            ...
                            <TableBody>
                                {stableSort<TRecord>(rows, c(order, orderBy))
                                    ...
                                    .map((row, index) => {
                                        const isItemSelected = isSelected(`${row[id_field]}-${index}`);
