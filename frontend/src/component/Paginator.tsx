...
export default class Paginator extends React.Component<IPaginatorProps, IPaginatorState > {
    ...
    render_center(pages: IPageButton[]) {
        ...
        for (let i = 0; i < pages.length; i++) {
            if (this.state.current_page == i) {
                b.push(<li key={i} className="current"><span className="show-for-sr">You're on page</span> {i + 1}</li>)
            } else {
                b.push(<li key={i}>
                    <a onClick={(evt) => {
                        this.onChangePage(evt, i)
                    }} href="#" aria-label={`Page ${i + 1}`}>{i + 1}</a>
                </li>)
            }
        }
        return b;
    ...
    render_previous() {
        ...
        return (
            <li><a className={prevCls} href="#" aria-label="Previous page" onClick={(evt) => {
    ...
    render_next() {
        ...
        return (
            <li><a className={nextCls} href="#" aria-label="Next page" onClick={(evt) => {
                this.onChangePage(evt, this.state.current_page + 1)
            }}>Next</a></li>
        )
    }
