...
import { Link as RouterLink, createLazyFileRoute, useRouteContext } from '@tanstack/react-router';
import { ContainerWithHeader } from '../../component/ContainerWithHeader.tsx';
import { ReportCreateForm } from '../../component/ReportCreateForm.tsx';
import { UserReportHistory } from '../../component/UserReportHistory.tsx';

export const Route = createLazyFileRoute('/_auth/report/')({
    component: ReportCreate
});

function ReportCreate() {
    const { user } = useRouteContext({ from: '/_auth/report/' });

    const canReport = useMemo(() => {
        return user.steam_id && user.ban_id == 0;
    }, [user.ban_id, user.steam_id]);
            ...
            <Grid xs={12} md={8}>
                <Stack spacing={2}>
                    ...
                    {!canReport && (
                        <ContainerWithHeader title={'Permission Denied'}>
                            ...
                            <ButtonGroup sx={{ padding: 2 }}>
                                <Button component={RouterLink} variant={'contained'} color={'primary'} to={`/ban/${user.ban_id}`}>
                                    Appeal Ban
                                </Button>
                            </ButtonGroup>
                        </ContainerWithHeader>
                    ...
                    <ContainerWithHeader title={'Your Report History'} iconLeft={<HistoryIcon />}>
                        <UserReportHistory steam_id={user.steam_id} />
                    </ContainerWithHeader>
                </Stack>
            </Grid>
            <Grid xs={12} md={4}>
