...
const validateSearch = makeSchemaState("duel_id");

export const Route = createFileRoute("/_guest/mge/2v2")({
	...
	head: ({ match }) => ({
		meta: [
			{
				name: "description",
				content: "MGE 2v2 Match History",
			},
			match.context.title("MGE 2v2 Match History"),
		],
	}),
});
...
function MGEOverall() {
	...
	const { data, isLoading, isError, isRefetching } = useQuery({
		...
		queryFn: async ({ signal }) => {
			...
			const winner2 = filterValue("winner2", search.columnFilters);
			const loser2 = filterValue("loser2", search.columnFilters);
	...
	const columns = useMemo(
		() => [
			...
			columnHelper.accessor("winner2", {
				grow: true,
				header: "Winner2",
				Cell: ({ row }) => (
					<PersonCell
						steam_id={row.original.winner2}
						avatar_hash={row.original.winner2_avatarhash}
						personaname={row.original.winner2_personaname}
					>
						<RouterLink
							style={{
								color:
						...
						>
							{row.original.winner2_personaname}
						</RouterLink>
					</PersonCell>
				),
			}),
			...
			columnHelper.accessor("loser2", {
				grow: true,
				header: "Loser2",
				Cell: ({ row }) => (
					<PersonCell
						steam_id={row.original.loser2}
						avatar_hash={row.original.loser2_avatarhash}
						personaname={row.original.loser2_personaname}
					>
						<RouterLink
							style={{
								color:
						...
						>
							{row.original.loser2_personaname}
						</RouterLink>
					</PersonCell>
				),
			}),
	...
	return (
		<Grid container>
			<Grid size={{ xs: 12 }}>
				<SortableTable table={table} title={"MGE 2v2 Match History"} />
			</Grid>
		</Grid>
	);
}
