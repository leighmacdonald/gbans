...
export const Route = createLazyFileRoute('/_authoptional/profile')({
...
function ProfilePage() {
    ...
    const { hasPermission } = useRouteContext({ from: '/_authoptional/profile' });
                ...
                <>
                    <Grid xs={12}>
                        {hasPermission(PermissionLevel.User) ? (
                            <PlayerStatsOverallContainer steam_id={data.player.steam_id} />
                        ) : (
                            <LoginPage />
                        )}
                    </Grid>
