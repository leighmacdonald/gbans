...
export const Route = createLazyFileRoute('/_authoptional/forums/thread/$forum_thread_id')({
...
const ThreadMessageContainer = ({
    ...
    const { hasPermission, userSteamID } = useRouteContext({ from: '/_authoptional/forums/thread/$forum_thread_id' });
    const theme = useTheme();
...
function ForumThreadPage() {
    ...
    const { hasPermission, permissionLevel } = useRouteContext({ from: '/_authoptional/forums/thread/$forum_thread_id' });
