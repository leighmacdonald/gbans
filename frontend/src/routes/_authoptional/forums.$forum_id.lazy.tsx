...
import { createLazyFileRoute, Link as RouterLink, useNavigate, useRouteContext } from '@tanstack/react-router';
...
import { PermissionDenied } from '../../component/PermissionDenied.tsx';
...
export const Route = createLazyFileRoute('/_authoptional/forums/$forum_id')({
    component: ForumPage
});
...
function ForumPage() {
    ...
    const { hasPermission } = useRouteContext({ from: '/_authoptional/forums/$forum_id' });
    ...
    const onNewThread = useCallback(async () => {
        try {
            ...
            navigate({ to: `/forums/thread/${thread.forum_thread_id}` });
