...
void OnPluginStart() {
    ...
    gbCorePort =
        AutoExecConfig_CreateConVar("gb_core_port", "6006", "Remote gbans port", FCVAR_NONE, true, 1.0, true, 65535.0);
    gbCoreServerKey = AutoExecConfig_CreateConVar("gb_core_server_key", "",
                                                  "GBans server key used to authenticate with the service", FCVAR_NONE);

    // In Game Tweaks
    gbDisableAutoteam = AutoExecConfig_CreateConVar(
        "gb_hide_connections", "1", "Dont allow the use of autoteam command", FCVAR_NONE, true, 0.0, true, 1.0);
    gbHideConnections = AutoExecConfig_CreateConVar(
        "gb_disable_autoteam", "1", "Dont show the disconnect message to users", FCVAR_NONE, true, 0.0, true, 1.0);

    // STV settings
    gbStvEnable =
        AutoExecConfig_CreateConVar("gb_stv_enable", "1", "Enable SourceTV", FCVAR_NONE, true, 0.0, true, 1.0);
    gbAutoRecord    = AutoExecConfig_CreateConVar("gb_auto_record", "1", "Enable automatic recording", FCVAR_NONE, true,
                                                  0.0, true, 1.0);
    gbStvMinplayers = AutoExecConfig_CreateConVar("gb_stv_minplayers", "1",
                                                  "Minimum players on server to start recording", _, true, 0.0);
    gbStvIgnorebots = AutoExecConfig_CreateConVar("gb_stv_ignorebots", "1", "Ignore bots in the player count",
                                                  FCVAR_NONE, true, 0.0, true, 1.0);
    gbStvTimestart  = AutoExecConfig_CreateConVar("gb_stv_timestart", "-1",
                                                  "Hour in the day to start recording (0-23, -1 disables)", FCVAR_NONE);
    gbStvTimestop   = AutoExecConfig_CreateConVar("gb_stv_timestop", "-1",
                                                  "Hour in the day to stop recording (0-23, -1 disables)", FCVAR_NONE);
    gbStvFinishmap = AutoExecConfig_CreateConVar("gb_stv_finishmap", "1", "If 1, continue recording until the map ends",
                                                 FCVAR_NONE, true, 0.0, true, 1.0);
    gbStvPath      = AutoExecConfig_CreateConVar("gb_stv_path", "stv_demos/active",
                                                 "Path to store currently recording demos", FCVAR_NONE);
    gbStvPathComplete = AutoExecConfig_CreateConVar("gb_stv_path_complete", "stv_demos/complete",
                                                    "Path to store complete demos", FCVAR_NONE);
...
public
void OnClientDisconnect_Post(int client) { CheckStatus(); }
...
public
void OnMapStart() { authenticateServer(); }
