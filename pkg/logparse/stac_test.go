...
`
	...
	entries, errParse := parser.Parse("stac_081024.log", strings.NewReader(testLog1))
	require.NoError(t, errParse)
	require.Len(t, entries, 9)
	require.Equal(t, logparse.SilentAim, entries[0].Detection)
	require.Equal(t, logparse.AimSnap, entries[1].Detection)
	require.Equal(t, logparse.CmdNumSpike, entries[2].Detection)
	require.Equal(t, logparse.BHop, entries[3].Detection)
	require.Equal(t, logparse.EyeAngles, entries[4].Detection)
	require.Equal(t, logparse.InvalidUserCmd, entries[5].Detection)
	require.Equal(t, logparse.OOBCVar, entries[6].Detection)
	require.Equal(t, logparse.CheatCVar, entries[7].Detection)
