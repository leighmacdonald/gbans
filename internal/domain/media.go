...
type MediaUsecase interface {
	Create(ctx context.Context, steamID steamid.SteamID, name string, mimeType string, content []byte, mimeTypesAllowed []string) (*Media, error)
	GetMediaByAssetID(ctx context.Context, uuid uuid.UUID, media *Media) error
	GetMediaByName(ctx context.Context, name string, media *Media) error
	GetMediaByID(ctx context.Context, mediaID int, media *Media) error
}

func NewMedia(author steamid.SteamID, name string, mime string, content []byte) (Media, error) {
