...
func toConfig(conf Config) *configv1.Config {
	return &configv1.Config{
		...
		Demo: &configv1.Demo{
			...
			CountLimit:     new(int64(conf.Demo.DemoCountLimit)),
			ParserUrl:      &conf.Demo.DemoParserURL,
		},
		Filters: &configv1.Filters{
			Enabled:        &conf.Filters.Enabled,
			WarningTimeout: new(int32(conf.Filters.WarningTimeout)),
			WarningLimit:   new(int32(conf.Filters.WarningLimit)),
			Dry:            &conf.Filters.Dry,
			PingDiscord:    &conf.Filters.PingDiscord,
			MaxWeight:      new(conf.Filters.MaxWeight),
			CheckTimeout:   new(int32(conf.Filters.CheckTimeout)),
			MatchTimeout:   new(int32(conf.Filters.MatchTimeout)),
		},
		Log: &configv1.Log{
		...
		Ssh: &configv1.SSH{
			...
			HostKeyStrategy: new(configv1.HostKeyStrategy(conf.SSH.HostKeyStrategy)),
			Password:        &conf.SSH.Password,
			UpdateInterval:  new(int32(conf.SSH.UpdateInterval)),
			Timeout:         new(int32(conf.SSH.Timeout)),
		...
		Anticheat: &configv1.Anticheat{
			...
			Duration:              new(int32(conf.Anticheat.Duration)),
			MaxAimSnaps:           new(int32(conf.Anticheat.MaxAimSnap)),
			MaxPsilent:            new(int32(conf.Anticheat.MaxPsilent)),
			MaxBhop:               new(int32(conf.Anticheat.MaxBhop)),
			MaxFakeAng:            new(int32(conf.Anticheat.MaxFakeAng)),
			MaxCmdNum:             new(int32(conf.Anticheat.MaxCmdNum)),
			MaxTooManyConnections: new(int32(conf.Anticheat.MaxTooManyConnections)),
			MaxOobVar:             new(int32(conf.Anticheat.MaxOOBVar)),
			MaxInvalidUserCmd:     new(int32(conf.Anticheat.MaxInvalidUserCmd)),
		},
	}
}
