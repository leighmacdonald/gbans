...
func (c *configRepository) Read(ctx context.Context) (domain.Config, error) {
	const query = `
		SELECT general_site_name, general_steam_key, general_mode, general_file_serve_mode, general_srcds_log_addr, general_srcds_log_addr_external, general_asset_url,
	...
	err := c.db.QueryRow(ctx, query).
		Scan(&cfg.General.SiteName, &cfg.General.SteamKey, &cfg.General.Mode, &cfg.General.FileServeMode, &cfg.General.SrcdsLogAddr, &cfg.General.SrcdsLogAddrExternal, &cfg.General.AssetURL,
...
func (c *configRepository) Write(ctx context.Context, config domain.Config) error {
	return c.db.DBErr(c.db.ExecUpdateBuilder(ctx, c.db.Builder().
		...
		SetMap(map[string]interface{}{
			...
			"general_srcds_log_addr_external":     config.General.SrcdsLogAddrExternal,
