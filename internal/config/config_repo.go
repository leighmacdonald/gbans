...
func (c *Repository) Read(ctx context.Context) (Config, error) {
	...
	err := c.QueryRow(ctx, query).
		Scan(&cfg.General.SiteName, &cfg.General.SiteDescription, &cfg.General.Mode, &cfg.General.FileServeMode, &cfg.General.SrcdsLogAddr, &cfg.General.AssetURL,
			...
			&cfg.Discord.SeedChannelID, &cfg.Discord.ChatLogChannelID,
...
func (c *Repository) Init(ctx context.Context) error {
...
func (c *Repository) Write(ctx context.Context, config Config) error {
	return database.Err(c.ExecUpdateBuilder(ctx, c.Builder().
		...
		SetMap(map[string]any{
			...
			"discord_chat_log_channel_id":         config.Discord.ChatLogChannelID,
			...
			"network_sdr_enabled":                 config.Network.SDREnabled,
		})))
}
