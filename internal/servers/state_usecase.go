...
func (s *StateUsecase) FindByCIDR(cidr *net.IPNet) []PlayerServerInfo {
	return s.Find("", steamid.SteamID{}, nil, cidr)
}

func (s *StateUsecase) FindByIP(addr net.IP) []PlayerServerInfo {
	return s.Find("", steamid.SteamID{}, addr, nil)
}

func (s *StateUsecase) FindByName(name string) []PlayerServerInfo {
	return s.Find(name, steamid.SteamID{}, nil, nil)
}

func (s *StateUsecase) FindBySteamID(steamID steamid.SteamID) []PlayerServerInfo {
	return s.Find("", steamID, nil, nil)
}

func (s *StateUsecase) Update(serverID int, update PartialStateUpdate) error {
...
func (s *StateUsecase) Find(name string, steamID steamid.SteamID, addr net.IP, cidr *net.IPNet) []PlayerServerInfo {
...
func (s *StateUsecase) OnFindExec(ctx context.Context, name string, steamID steamid.SteamID, ip net.IP, cidr *net.IPNet, onFoundCmd func(info PlayerServerInfo) string) error {
...
func (s *StateUsecase) Kick(ctx context.Context, target steamid.SteamID, reason string) error {
	if !target.Valid() {
...
func (s *StateUsecase) KickPlayerID(ctx context.Context, targetPlayerID int, targetServerID int, reason string) error {
...
func (s *StateUsecase) Silence(ctx context.Context, target steamid.SteamID, reason string,
) error {
	if !target.Valid() {
		return domain.ErrInvalidTargetSID
	}
